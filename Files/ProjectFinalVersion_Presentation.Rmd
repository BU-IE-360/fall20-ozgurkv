...
author: "IE360 - Fall 2020 - Group 4: Safiye Şahin - Fatma İlayda Tutal - Özgür Kaan Varlık"
output: 
...
library(readxl)
library(gridExtra)
consumption <- read_excel("C:/Users/ozgur/Desktop/Documents/IE 360/Project/bulk_consumption_with_temp.xls")
head(consumption,10)
setDT(consumption)
...
There are several factors affecting the electricity consumption of a country. One aspect for sure to be considered is the industrial developments in that country. In an underdeveloped country, since a lot of people don't have the access to electricity, they won't be able to consume it.  

Another very important aspect is the overall climate in that country. When electricity consumption per capita is investigated, it can be seen that a country that is generally considered to have a colder climate and harsher weather conditions tends to have a higher consumption per capita. 

Population is also an important indicator of the level of electricity consumption. If there are more people living in a country, it is very much expected that it has a higher overall electricity consumption.

All in all, there are many aspects, some of which have not been discussed such as the level of insulation of houses in that country, that have an impact on the electricity consumption of a country. For the case of Turkey, it is for sure that the weather conditions will have a great influence, since Turkey is a country where all 4 seasons can be experienced.
...
```{r, layout="l-body-outset"}
library(knitr)
kable(tail(daily_consumption_shown2, 5))
```
...
#hourly <- consumption[c(35377:35712),]       2 haftalık periyot
#hourly <- consumption[c(35041:35712),]       4 haftalık periyot
...
* [**R Markdown File**](https://github.com/BU-IE-360/fall20-ozgurkv/blob/master/Files/ProjectFinalVersion.Rmd)
* [**Code for Submissions**](https://github.com/BU-IE-360/fall20-ozgurkv/blob/master/Files/SubmissionCode.R)
